...
#kerala bus
kerala=[]
df_k=pd.read_csv("route_link_data/df_k.csv")
...
#south bengal bus
sbengal=[]
df_s=pd.read_csv("route_link_data/df_sb.csv")
for i,r in df_s.iterrows():
    sbengal.append(r["Route_name"])
    
#uttar pradesh bus
up=[]
df_u=pd.read_csv("route_link_data//df_up.csv")
...
st.set_page_config(layout="wide",page_icon=":material/directions_bus:",page_title="Red Bus Project",initial_sidebar_state="expanded")
...
#clr = st.color_picker("pic a color")
#st.write(clr)
...
with st.sidebar:
    ...
    menu = option_menu(
        ...
        styles={
            ...
            #"nav-link-selected": {"background-color": "#113332","font-size":"20px"}
        }
    )


if menu=="Home":
    st.title(":blue[:material/analytics:] :orange[Redbus Data Scraping with Selenium  & Dynamic Filtering using Streamlit]")
    st.text("")
    st.subheader(" ")
    st.markdown(""" ### :green[:material/tooltip:] :red[*OBJECTIVE*]
...
if menu=="Bus Routes":
    ...
    with col2:
        select_rating = st.slider("Choose rating", min_value=1, max_value=5, value=2)
    with col1:
        TIME=st.time_input("select the time",step = 60)  
    ...
    if state=="Kerala":
        ...
        def type_and_fare(bus_type, fare_range,rate_range):
            conn=mysql.connector.connect(host="localhost",user="root",password="Rameez22!",database="redbus",port=3306)
            ...
            if rate_range == 5:
                ...
                rate_min, rate_max = 3.1, 4.1
            elif rate_range == 3:
                rate_min, rate_max = 2.1, 3.1
            elif rate_range == 2:
                rate_min, rate_max = 1.1, 2.1
            elif rate_range == 1:
                rate_min, rate_max = 0, 1.1
            ...
            sqlquery= f""" 
                ...
                AND rating BETWEEN {rate_min} and {rate_max}
